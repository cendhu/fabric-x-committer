...
func (e *coordinatorTestEnv) createNamespaces(t *testing.T, blkNum int, nsIDs ...string) {
	...
	for _, tx := range blk.Txs {
		...
		tx.Content.Signatures = make([]*protoblocktx.SignatureWithIdentity, len(tx.Content.Namespaces))
	}
		...
		Txs: []*protocoordinatorservice.Tx{
			{
				...
				Content: &protoblocktx.Tx{
					...
					Signatures: make([]*protoblocktx.SignatureWithIdentity, 2),
				},
			},
		},
	})
...
func TestCoordinatorServiceDependentOrderedTxs(t *testing.T) {
	...
	for _, tx := range b1.Txs {
		tx.Content.Signatures = []*protoblocktx.SignatureWithIdentity{{Signature: []byte("dummy")}}
	}
	...
	err := env.csStream.Send(&protocoordinatorservice.Batch{
		Txs: []*protocoordinatorservice.Tx{{
			...
			Content: &protoblocktx.Tx{
				...
				Signatures: make([]*protoblocktx.SignatureWithIdentity, 1),
			},
		}},
	})
	require.NoError(t, err)
		...
		Txs: []*protocoordinatorservice.Tx{
			...
			},
			{
				...
				Content: &protoblocktx.Tx{
					...
					Signatures: []*protoblocktx.SignatureWithIdentity{{Signature: []byte("dummy")}},
				},
			},
			{
				...
				Content: &protoblocktx.Tx{
					...
					Signatures: make([]*protoblocktx.SignatureWithIdentity, 3),
				},
			},
			{
				Ref: types.TxRef("tx1", 2, 5),
